<?php
// Model xử lý dữ liệu tài khoản người dùng (tìm kiếm, đăng ký)
class AccountModel
{
    private $conn;
    ...
    // Tìm tài khoản theo username, trả về object hoặc false
    ...
    public function save($username, $fullName, $phone, $password, $role = 'user')
    {
        ...
        // Làm sạch dữ liệu và băm mật khẩu
        ...
        // Kiểm tra cột mở rộng (fullname, phone) có tồn tại không
        ...
        if (in_array('fullname', $existingCols, true)) {
            $columns[] = 'fullname'; $values[] = ':fullname'; $params[':fullname'] = $fullName;
        }
        if (in_array('phone', $existingCols, true)) {
            $columns[] = 'phone'; $values[] = ':phone'; $params[':phone'] = $phone;
        }
